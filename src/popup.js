...
async function exportData(type) {
        ...
        func: (exportType) => {
            ...
            if (exportType === 'efficiency') {
                ...
                function calcEfficiency(item, cols) {
                    ...
                    const efficiency = item.size_total > 0 ? sum / item.size_total : 0;
    ...
    if (!result) {
        alert('No se encontraron datos. Asegúrate de estar en el juego.');
        return;
    }
    ...
    if (result.type === 'excel') {
        ...
        try {
            ...
            alert(`Excel generado: ${result.filename}\n${stats.sheetCount} hojas, ${stats.totalRows} edificios`);
        } catch (e) {
    ...
    if (result.type === 'catalog') {
        ...
        try {
            ...
            alert(`Catálogo generado: ${result.filename}\n${stats.sheetCount} hojas, ${stats.totalRows} edificios`);
        } catch (e) {
            console.error('Error generando Catálogo:', e);
            alert('Error generando Catálogo: ' + e.message);
        }
        return;
    }
}
